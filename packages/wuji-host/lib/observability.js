...
export function createStatusTool(sessionProjections){return{name:'wuji_status',description:'读取当前 Session 的无极军团投影状态摘要。',parameters:{type:'object',properties:{},required:[]},output:{schema:{type:'object'},render(_a,v){return[{type:'text',text:JSON.stringify(v,null,2)}]}},isConcurrencySafe(){return true},async execute(_args,exec){if(!exec.agent?.session)throw new Error('状态读取需要 Session');const snap=sessionProjections.snapshot(exec.agent.session);const values=snap.values||{};return{asOfSeq:snap.asOfSeq,keys:Object.keys(values).filter(k=>k.startsWith('wuji.')),task:values['wuji.telemetry']?.task||null,activeRequirement:values['wuji.requirement']?.active?.id||null,activeTask:values['wuji.task']?.active?.taskId||null}}}}
