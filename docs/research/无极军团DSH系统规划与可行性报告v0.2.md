# 无极军团 × DeepSeek Harness 系统规划与可行性报告（含上下文图谱补强）
...
> v0.2 变更：补充第 5.4 节"上下文图谱（稀疏投影）"，并同步更新路线与结论。
...
## 1. 组织架构（以确认的完整体系为准，不用 2.1 的压缩表述）
...
| 上下文图谱 | `sessionProjections` 原生投影服务（见 5.4） | 现成底座 |
...
- **DSH 原生投影服务**：`sessionProjections`（`ctx.sessionProjections`）——事件驱动投影单元表 + 检查点/恢复/持久化；goal、todo、token-meter、subagent、session-title、plan-mode 均已在用。
- **无极资产**：17 个能力包 manifest（触发词/生命周期/探针/fallback）、`wuji.exe` 确定性路由（实测可运行）、行为探针脚本、融合基因组（presentation/interaction 两个物种）、3.0 图谱与进化主帅规则文档、2.1 图谱投影命令（requirement/execution-project，实测有严格调用链）。
...
| G7 | 无无极领域投影单元 | `sessionProjections` 机制现成，但"需求单元/执行节点/官员建议"三个域尚未注册 |
...
| `wuji-projection`（插件） | 宿主 | 三个投影单元注册：需求单元 / 执行节点 / 官员建议（见 5.4） |
...
### 5.4 上下文图谱（关键设计：每角色一表、每次只投影有限内容）

> 这是"最后一次改造"的核心机制，3.0 蓝图称"双图谱 + 稀疏投影"，2.1 已落地为 `requirement-graph` / `execution-graph` + `requirement-project` / `execution-project` + `conversation-link`。**核心原理：长期交流沉淀为结构化状态（图谱/表格），每次模型调用只重建"当前单元 + 必要依赖 + 版本指针"，不重放完整聊天。**

| 角色 | 自己的表（图谱） | 每次调用投影的内容 | 写入者 |
|---|---|---|---|
| 阿极 | 需求图、决策图、对话证据索引 | 当前活跃需求单元 + 必要依赖 + 版本指针 + 当前消息 | 阿极（唯一） |
| 参谋本部 | 执行图、验收证据图、任务尝试账本 | 当前执行节点 + 依赖 + 契约 + 回执 | 参谋本部（唯一） |
| 进化主帅 | 能力谱系融合图、全局经验图 | 命中事件对应的经验节点 | 进化主帅 |
| 独立官员 | 隔离观点图（按次创建） | 同一事实快照 + 自己的职权模板 | 官员（只建议） |

2.1 的投影命令均带字节预算（如 `requirement-project --max-bytes 4096`），并强制来源引用登记（`conversation-link` 后才可入图）——实测 wuji.exe 对无来源引用的裸调用直接拒绝，说明这套机制有严格调用链，不是摆设。

**DSH 侧的对应实现（关键优势）**：DSH 原生就有框架级投影服务 **`sessionProjections`**（`ctx.sessionProjections`）——订阅 `session/event`、纯函数折叠、schema 校验视图、checkpoint/restore/持久化投影缓存。`dsh-goal`（goal 服务）、`dsh-tool-todo`、`dsh-token-meter`（token 用量/上下文压力）、`dsh-subagent`（计时/身份）、`dsh-session-title`、`dsh-plan-mode` 全都注册了自己的投影单元。**因此本系统不需要移植 Go 的图谱存储：直接在 `sessionProjections` 上注册"需求单元 / 执行节点 / 官员建议"三个投影单元，即可获得 2.1 设计的全部收益**（事件驱动、版本化、持久化、UI 可读），且与宿主原生机制同构。

> 落地形态（规划）：`wuji-projection` 插件内 `ctx.sessionProjections.register({ key, schema, init, apply, view, stateVersion })`，三个 key 分别对应阿极需求表、参谋本部执行分配表、官员建议表；每次模型调用前由 `systemPrompt.variable()` 注入"当前活跃单元投影"（带字节预算），完整历史仍留在会话日志冷存储。
...
| 阶段 | 内容 | 产出 | 依赖 |
|---|---|---|---|
| **P0 地基** | ① 能力注册表 v1（导入 17 manifest + 本机 27 lark + dashi-ppt + MCP 清单）② 现有 skill description 语义化 ③ wuji-preset 骨架（参谋本部 persona + 路由热路径 SKILL）④ **wuji-projection 三个投影单元先行注册（空 schema 骨架）** | 注册表 JSON + preset 目录 + 投影插件骨架 | 无 |
...
| **P3 官员** | wuji-officers：契约触发 + 隔离 subagent 审查席 + 建议回执流程（建议写入投影单元） | 官员机制 | P1 |
...
**上下文图谱在路线中的位置**：其宿主机制（`sessionProjections`）在 **P0** 阶段即可随 wuji-preset 先注册三个投影单元（需求/执行/官员建议，先空 schema 后填充），**P2-P4** 随各能力落地逐步写实。因为它完全复用宿主机制，不增加独立存储，风险最低、收益最直接（长会话上下文压缩 + UI 可读）。
...
| 上下文膨胀（技能/候选堆积） | 中 | 沿用 2.1 预算 + 只装载被路由选中的能力；记忆走经验图；**会话长文走 sessionProjections 稀疏投影** |
| 与现有 27 个 lark 技能冲突 | 低 | 飞书域融合以"统一入口 + 域路由"方式收敛，不删除已验证的官方 lark-cli 通道 |
| 投影单元 schema 与未来流程不匹配 | 低 | P0 先注册空 schema 骨架并标 stateVersion=1；演化时递增 stateVersion 走宿主恢复机制，不破坏历史 |
...
3. **上下文图谱机制确认有用且 DSH 已备好底座**：3.0 的"每角色一表、稀疏投影"在 DSH 上由 `sessionProjections` 原生承载，无需移植 Go 图谱存储；它是对抗长会话上下文膨胀的关键，也天然支持 UI 展示（goal/todo/token 都用同一机制）。
4. **最优先的两步**：P0 能力注册表 + wuji-projection 骨架（把 17 manifest + 本机能力统一登记，同时注册三个投影单元）+ P1 wuji-router（把路由做成宿主插件）。这些不动任何现有技能，纯增量，风险最低。
5. **需要外部融合的首选**：DSH 官方 MCP 客户端（MCP 通道）、社区插件集市（来源池）、记忆插件蒸馏（经验沉淀）、semantic-router 思路（路由增强）；飞书社区 MCP 与 A2A 协议列为待证/观望。
