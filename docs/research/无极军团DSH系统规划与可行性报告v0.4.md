...
> 依据：**[无极军团设计资产清点与DSH适配矩阵](./无极军团设计资产清点与DSH适配矩阵.md)**（v1.2，权威基线，以下裁决统一引用其行号，如 C1/O1/E2 + 教训四/教训五）+ DeepSeek Harness 运行时实测 + 2026 年外部生态全网调研。
> v0.4 变更：① 依据矩阵升级到 v1.2；② 新增 §0.5"两条最高优先级原则"（前置限制优先于后置验证；白帽反驳是默认开启的前置机制）；③ P0 阶段增加"生成前锁定目标 + 白帽视角前置"作为强制起步项。

---

## 0.5 两条最高优先级原则（用户 2026-08-15 明确指出，全系统最高约束）

1. **前置限制优先于后置验证**：一个任务慢，是因为把质量责任放在"生成后验证→不合格→重做"的循环上（每次都是分钟级模型往返）。正确做法是**生成前就限制住**：scope/边界/约束/验收标准/禁止项在任务启动前写进契约并锁定（矩阵教训四）。新增任何验证步骤前必须论证"为什么不能用前置限制替代"。
2. **白帽反驳是默认开启的前置机制**：阿极（模型）天然倾向迎合用户；没有反方检查，用户的想法即使有缺陷也会被执行。白帽（从用户利益出发找反例/风险/错误假设/遗漏）必须在**方案确认前**强制输出一次，而不是生成后审查（矩阵教训五）。"迎合用户、不提反例"的输出判定不合格。

> 这两条是矩阵 v1.2 的教训四/教训五，优先级高于本报告其他所有设计；任何阶段落地冲突时以这两条为准。
...
> **两条最高优先级原则（§0.5）贯穿所有阶段**：① 生成前锁定目标（goal boundary lock），不做后置验证循环；② 方案确认前强制白帽视角，禁止迎合。

| 阶段 | 内容 | 产出 | 依赖 |
|---|---|---|---|
| **P0 地基** | ① wuji-registry v1（导入 17 manifest + 本机 27 lark + dashi-ppt + MCP 清单）② skill description 语义化 ③ wuji-preset 骨架（**含白帽强制义务 persona**：确认方案前必须输出假设/反例/风险/备选）④ **wuji-projection 三个投影单元先行注册（空 schema）** ⑤ **goal boundary lock 契约模板**（scope/边界/约束/验收/禁止项，启动前锁定，教训四） | 注册表 + preset + 投影骨架 + 契约模板 | 无 |
| **P1 路由** | wuji-router（确定性路由 + capability-mount）+ wuji-gate 门禁（**前置门禁**，非后置审查）接入 approval 栈 | 路由插件 | P0 |
| **P2 融合** | wuji-evolution：能力审计 workflow → 首批融合（飞书域/演示域/内容提取域）→ **互补矩阵 + 同 fixture 对照**（教训 §0.5 走样一） | 首批融合插件 | P1 |
| **P3 官员** | wuji-officers：契约触发 + 隔离 subagent 审查席；**白帽=默认前置**，其余官员按需 | 官员机制 | P1 |
